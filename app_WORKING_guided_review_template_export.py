...
with batch_tab:
    ...
    st.warning(
        ...
        "Topical fields are marked for manual review in batch mode."
    )
            ...
            if url_column not in df.columns:
                ...
                if batch_df.empty:
                    ...
                    for i, (_, input_row) in enumerate(batch_df.iterrows(), start=1):
                        ...
                        content_draft = draft_content_association_from_metadata(
                            assessment
                        )
                        ...
                        manual = {
                            ...
                            "content_association": format_guided_review_result(
                                category="Content Association",
                                finding="Needs Review",
                                reviewed_links=["Metadata"],
                                notes=content_draft,
                            ),
                            "mainstream": format_guided_review_result(
                                category="Mainstream",
                                finding="Needs Review",
                                reviewed_links=[],
                                notes="Batch mode requires analyst review.",
                            ),
                            "adversarial": format_guided_review_result(
                                category="Adversarial",
                                finding="Needs Review",
                                reviewed_links=[],
                                notes="Batch mode requires analyst review.",
                            ),
                            "reputation": format_guided_review_result(
                    ...
                    if template_file is not None:
                        ...
                        output_path = (
                            OUTPUT_DIR
                            / f"MA_Tier0_Checklist_BATCH_DRAFT_FILLED_{timestamp}.xlsx"
                        )
                        ...
                        st.success(
                            "Batch draft rows written into a copy of the uploaded checklist template."
                        )

                        with open(output_path, "rb") as f:
                            st.download_button(
                                "Download Filled Batch Checklist Copy",
                                f,
                                file_name=output_path.name,
                            )
                        ...
                        output_path = (
                            OUTPUT_DIR
                            / f"MA_Tier0_Batch_Draft_Results_{timestamp}.xlsx"
                        )

                        write_draft_excel(rows, str(output_path))

                        st.success("Standalone batch draft Excel file created.")
